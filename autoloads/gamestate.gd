...
func attempt_host_steam_game(lobby_type):
...
func host_steam_mp_game(hosting_lobby_id : int) -> void:
	...
	SteamManager.steam_api.setLobbyData(hosting_lobby_id, "name", "%s's Lobby" % [player_name])
...
func _on_steam_lobby_created(connection: int, this_lobby_id: int) -> void:
	print("Lobby creation attempt returned.")
	if connection != SteamManager.steam_api.RESULT_OK:
		...
		# Set some lobby data
		SteamManager.steam_api.setLobbyData(lobby_id, "name", "Default Name Lobby")

		# Allow P2P connections to fallback to being relayed through Steam if needed
		var set_relay: bool = SteamManager.steam_api.allowP2PPacketRelay(true)
...
func display_specific_steam_lobby_creation_error(connection_failed_reason_code : int) -> void:
	...
	match connection_failed_reason_code:
		SteamManager.steam_api.RESULT_FAIL: fail_reason = "This lobby no longer exists."
		SteamManager.steam_api.RESULT_TIMEOUT: fail_reason = "TThe message was sent to the Steam servers, but it didn't respond."
		SteamManager.steam_api.RESULT_LIMIT_EXCEEDED: fail_reason = "Your game client has created too many lobbies and is being rate limited."
		SteamManager.steam_api.RESULT_ACCESS_DENIED: fail_reason = "Your game isn't set to allow lobbies, or your client does not rights to play the game!"
		SteamManager.steam_api.RESULT_NO_CONNECTION: fail_reason = "Your Steam client doesn't have a connection to the back-end."
...
func join_steam_lobby(this_lobby_id: int) -> void:
	...
	SteamManager.steam_api.joinLobby(this_lobby_id)
	
func _on_steam_lobby_join_requested(this_lobby_id: int, friend_id: int) -> void:
	# Get the lobby owner's name
	var owner_name: String = SteamManager.steam_api.getFriendPersonaName(friend_id)
...
func _on_steam_lobby_joined(this_lobby_id: int, _permissions: int, _locked: bool, response: int) -> void:
	...
	if response == SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_SUCCESS:
		...
		if SteamManager.steam_api.getLobbyOwner(lobby_id) != SteamManager.steam_api.getSteamID():
	...
	# Else it failed for some reason
		...
		match response:
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_DOESNT_EXIST: fail_reason = "This lobby no longer exists."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_NOT_ALLOWED: fail_reason = "You don't have permission to join this lobby."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_FULL: fail_reason = "The lobby is now full."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_ERROR: fail_reason = "YOu should not be seeing this message!"
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_BANNED: fail_reason = "You are banned from this lobby."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_LIMITED: fail_reason = "You cannot join due to having a limited account."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_CLAN_DISABLED: fail_reason = "This lobby is locked or disabled."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_COMMUNITY_BAN: fail_reason = "This lobby is community locked."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_MEMBER_BLOCKED_YOU: fail_reason = "A user in the lobby has blocked you from joining."
			SteamManager.steam_api.CHAT_ROOM_ENTER_RESPONSE_YOU_BLOCKED_MEMBER: fail_reason = "A user you have blocked is in the lobby."
...
func get_lobby_members() -> void:
	...
	var num_of_members: int = SteamManager.steam_api.getNumLobbyMembers(lobby_id)

	# Get the data of these players from Steam
	for this_member in range(0, num_of_members):
		# Get the member's Steam ID
		var member_steam_id: int = SteamManager.steam_api.getLobbyMemberByIndex(lobby_id, this_member)

		# Get the member's Steam name
		var member_steam_name: String = SteamManager.steam_api.getFriendPersonaName(member_steam_id)
...
func _on_steam_lobby_chat_update(_this_lobby_id: int, change_id: int, _making_change_id: int, chat_state: int) -> void:
	...
	var changer_name: String = SteamManager.steam_api.getFriendPersonaName(change_id)

	# If a player has joined the lobby
	if chat_state == SteamManager.steam_api.CHAT_MEMBER_STATE_CHANGE_ENTERED:
		print("%s has joined the lobby." % changer_name)

	# Else if a player has left the lobby
	elif chat_state == SteamManager.steam_api.CHAT_MEMBER_STATE_CHANGE_LEFT:
		print("%s has left the lobby." % changer_name)

	# Else if a player has been kicked
	elif chat_state == SteamManager.steam_api.CHAT_MEMBER_STATE_CHANGE_KICKED:
		print("%s has been kicked from the lobby." % changer_name)

	# Else if a player has been banned
	elif chat_state == SteamManager.steam_api.CHAT_MEMBER_STATE_CHANGE_BANNED:
...
func leave_steam_lobby() -> void:
	...
	if lobby_id != 0:
		...
		SteamManager.steam_api.leaveLobby(lobby_id)
		...
		for this_member in lobby_members:
			...
			if this_member['steam_id'] != steam_id:
				...
				SteamManager.steam_api.closeP2PSessionWithUser(this_member['steam_id'])
...
func _on_steam_network_messages_session_request(remote_id: int) -> void:
	...
	var this_requester: String = SteamManager.steam_api.getFriendPersonaName(remote_id)
	print("%s is requesting a P2P session" % this_requester)

	# Accept the P2P session; can apply logic to deny this request if needed
	SteamManager.steam_api.acceptSessionWithUser(remote_id)
...
func send_steam_p2p_packet(this_target: int, packet_data: Dictionary) -> void:
	...
	var send_type: int = SteamManager.steam_api.P2P_SEND_RELIABLE
	var channel: int = 0
	...
	if this_target == 0:
		...
		if lobby_members.size() > 1:
			...
			for this_member in lobby_members:
				if this_member['steam_id'] != steam_id:
					SteamManager.steam_api.sendP2PPacket(this_member['steam_id'], this_data, send_type, channel)

	# Else send it to someone specific
	else:
		SteamManager.steam_api.sendP2PPacket(this_target, this_data, send_type, channel)
			
func steam_signal_setup() -> void:
	SteamManager.steam_api.join_requested.connect(_on_steam_lobby_join_requested)
	#SteamManager.steam_api.lobby_chat_update.connect(_on_lobby_chat_update) #No chatroom feature enabled.
	SteamManager.steam_api.lobby_created.connect(_on_steam_lobby_created)
	#SteamManager.steam_api.lobby_data_update.connect(_on_lobby_data_update)
	#SteamManager.steam_api.lobby_invite.connect(_on_lobby_invite)
	SteamManager.steam_api.lobby_joined.connect(_on_steam_lobby_joined)
	#SteamManager.steam_api.lobby_match_list.connect(_on_lobby_match_list)
	#SteamManager.steam_api.lobby_message.connect(_on_lobby_message)
	SteamManager.steam_api.persona_state_change.connect(_on_steam_persona_change)
	SteamManager.steam_api.network_messages_session_request.connect(_on_steam_network_messages_session_request)
	SteamManager.steam_api.network_messages_session_failed.connect(_on_steam_network_messages_session_failed)
...
func steam_playername_integration():
	player_id = SteamManager.steam_api.getSteamID()
	player_name = SteamManager.steam_api.getFriendPersonaName(player_id)
...
func execute_post_boot_steam_framework_methods() -> void:
	...
	establish_steam_multiplayer_peer()
