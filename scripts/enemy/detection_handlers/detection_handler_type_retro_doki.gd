...
func check_for_priority_target(force: bool = false) -> int:
	...
	if world_data.is_tile(world_data.tiles.BREAKABLE, self.enemy.position): return abiltiy.NONE
	...
	if is_on && self.enemy.cooldown_done && is_safe_to && (force || rand_val < trigger_chance_bomb + trigger_chance_breakable):
		if (force && rand_val < 1 / (trigger_chance_bomb + trigger_chance_breakable) * trigger_chance_bomb) || rand_val < trigger_chance_bomb:
