...
enum abiltiy { NONE, BOMB, DANCE }

@onready var enemy: Enemy = get_parent()
@export var trigger_chance_bomb: float = 0.05
@export var trigger_chance_dance: float = 0.05
...
func check_for_priority_target(force: bool = false) -> int:
	...
	if is_on && self.enemy.cooldown_done && is_safe_to && (force || rand_val < trigger_chance_bomb + trigger_chance_dance):
		if (force && rand_val < 1 / (trigger_chance_bomb + trigger_chance_dance) * trigger_chance_bomb) || rand_val < trigger_chance_bomb:
			curr_ability = abiltiy.BOMB
			return abiltiy.BOMB
		else:
			curr_ability = abiltiy.DANCE
			return abiltiy.DANCE
