...
static var texture_dict: Dictionary = {
	[StageDataUI.tile_type.UNBREAKABLE]: [[tileset_texture, Rect2i(192, 0, 32, 32)], null],
	[StageDataUI.tile_type.SPAWNPOINT]: [[spawnpoint_texture, null], null],
...
func apply_texture(texture_data: Array = []) -> void:
	if texture_data == []:
		...
		return
	if texture_dict.has(texture_data):
		main_texture.texture = arr_2_atlas(texture_dict[texture_data][0])
		if len(texture_dict[texture_data]) > 1 && texture_dict[texture_data][1] != null:
			sub_texture.texture = arr_2_atlas(texture_dict[texture_data][1])
	elif texture_dict.has([texture_data[0], ""]): #unimplemented enemy
		main_texture.texture = arr_2_atlas(texture_dict[[texture_data[0], ""]][0])
		if len(texture_dict[[texture_data[0], ""]]) > 1 && texture_dict[[texture_data[0], ""]][1] != null:
			sub_texture.texture = arr_2_atlas(texture_dict[[texture_data[0], ""]][1])
	elif texture_dict.has([texture_data[0], -1]): #unimplemented pickup
		main_texture.texture = arr_2_atlas(texture_dict[[texture_data[0], -1]][0])
		if len(texture_dict[[texture_data[0], -1]]) > 1 && texture_dict[[texture_data[0], -1]][1] != null:
			sub_texture.texture = arr_2_atlas(texture_dict[[texture_data[0], -1]][1])
	else:
