...
func _generate_breakables(breakable_table: BreakableTable):
	for breakable_entry in breakable_table.breakables:
		...
		assert(spawnpoints.all(func (s: Vector2i): return s != corrected_pos), "spawnpoints and breakables position should be mutualy exclusive " + str(spawnpoints) + " : " + str(breakable_entry.coords))
